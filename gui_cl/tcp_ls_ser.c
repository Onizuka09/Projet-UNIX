...
{
  printf("========================================\n");
  printf("RUNNING LIST FILES SERVICE \n");
  printf("========================================\n");
  ...
  service_socket = SafeInitSocketServer(PORT_LIST_DIR_SERVICE);
  safeListen(service_socket);

  LOG_INFO("Service LIST FILES started. Waiting for connections...");
...
void *host_handler(void *socket_desc)
{
  ...
  if (message_rcv.serviceType == LISTS_DIR)
  {
