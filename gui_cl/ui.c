...
{
    GooeyLabel *titleLabel = GooeyLabel_Add(&win, "Authentication Form: ", FONT_SIZE + 0.2f, 10, 20); // Adjusted to start from (20, 20)

    // Layout for Labels
    GooeyLayout *labelLayout = GooeyLayout_Create(&win, LAYOUT_VERTICAL, 20, 100, 100, 80); // Increased y-position for more space
    // Add Labels
    userLable = GooeyLabel_Add(&win, "Enter Username:", FONT_SIZE, 0, 0);
    passwdLabel = GooeyLabel_Add(&win, "Enter Password:", FONT_SIZE, 0, 0);
    ...
    GooeyLayout *textboxLayout = GooeyLayout_Create(&win, LAYOUT_VERTICAL, 140, 100, 120, 80); // Increased y-position for more space
    // Add Text Boxes
    usernameIN = GooeyTextBox_Add(&win, 0, 0, 120, 30, "", NULL); // Width matches layout width
    passwdIn = GooeyTextBox_Add(&win, 0, 0, 120, 30, "", NULL);
    ...
    GooeyLayout *combinedLayout = GooeyLayout_Create(&win, LAYOUT_HORIZONTAL, 20, 100, 250, 100); // Increased y-position for more space
    ...
    GooeyButton *addButton = GooeyButton_Add(&win, "Authenticate", 20, 180, 100, 30, onAuthenticate); // Adjusted y-position for more space

    // Vertical Layout to Wrap All Components
    GooeyLayout *mainLayout = GooeyLayout_Create(&win, LAYOUT_VERTICAL, 20, 20, 300, 220); // Increased height for more space
...
void create_services_layout()
{
    ...
    GooeyLayout *titleLayout = GooeyLayout_Create(&win, LAYOUT_HORIZONTAL, 20, 20, 400, 50);
    GooeyLabel *titleLabel = GooeyLabel_Add(&win, "Services", FONT_SIZE + 0.2f, 0, 0);
    GooeyLayout_AddChild(titleLayout, titleLabel);

    // Connection Status Label (Top-Right Corner)
    connectionStatusLabel = GooeyLabel_Add(&win, "Not Connected", FONT_SIZE + 0.15f, 350, 20);
    ...
    GooeyLayout *clientIDLayout = GooeyLayout_Create(&win, LAYOUT_HORIZONTAL, 20, 60, 400, 50);
    GooeyLabel *clientIDLabel = GooeyLabel_Add(&win, "Client ID:", FONT_SIZE + 0.15f, 0, 0);
    clientIDValue = GooeyLabel_Add(&win, "-1", FONT_SIZE + 0.15f, 50, 0); // Placeholder for Client ID
    ...
    GooeyLayout *verticalLayout1 = GooeyLayout_Create(&win, LAYOUT_VERTICAL, 20, 120, 100, 200);
    GooeyLabel *dateLabel = GooeyLabel_Add(&win, "Date & Time Service:", FONT_SIZE, 0, 0);
    GooeyLabel *elapsedTimeLabel = GooeyLabel_Add(&win, "Elapsed Time  Serivce:", FONT_SIZE, 0, 0);
    GooeyLabel *lsDirsLabel = GooeyLabel_Add(&win, "Ls dirs Service:", FONT_SIZE, 0, 0);
    GooeyLabel *catFilesLabel = GooeyLabel_Add(&win, "Cat files Service :", FONT_SIZE, 0, 0);
    ...
    GooeyLayout *verticalLayout2 = GooeyLayout_Create(&win, LAYOUT_VERTICAL, 140, 120, 200, 200);
    setDateLabel = GooeyLabel_Add(&win, "Empty", FONT_SIZE, 0, 0);        // Output for Date
    setElapsedTimeLabel = GooeyLabel_Add(&win, "Empty", FONT_SIZE, 0, 0); // Output for Elapsed Time
    pathTextBox = GooeyTextBox_Add(&win, 0, 0, 180, 25, "", NULL);        // Path input for Ls dirs
    fileNameTextBox = GooeyTextBox_Add(&win, 0, 0, 180, 25, "", NULL);    // Filename input for Cat files
    ...
    GooeyLayout *verticalLayout3 = GooeyLayout_Create(&win, LAYOUT_VERTICAL, 360, 120, 100, 200);
    dateButton = GooeyButton_Add(&win, "Get Date&Time Service", 0, 0, 50, 25, onDateAction);                  // Action for Date
    elapsedTimeButton = GooeyButton_Add(&win, "Get Elapsed Time Service", 0, 0, 50, 25, onElapsedTimeAction); // Action for Elapsed Time
    lsDirsButton = GooeyButton_Add(&win, "Get Ls Service", 0, 0, 50, 25, onLsDirsAction);                     // Action for Ls dirs
    catFilesButton = GooeyButton_Add(&win, "Get Cat Service ", 0, 0, 50, 25, onCatFilesAction);               // Action for Cat files
    ...
    GooeyLayout *mainLayout = GooeyLayout_Create(&win, LAYOUT_HORIZONTAL, 20, 100, 500, 300);
    ...
    resultList = GooeyList_Add(&win, 20, 410, 500, 400, NULL); // Large textbox for displaying results
    ...
    END_BTN = GooeyButton_Add(&win, "Close", 10, 840, 50, 30, OnCloseAction); // Action for Date
}
