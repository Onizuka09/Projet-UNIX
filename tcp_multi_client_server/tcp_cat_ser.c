...
{
  printf("========================================\n");
  printf("RUNNING CAT CONTENT FILES SERVICE \n");
  printf("========================================\n");
  ...
  service_socket = SafeInitSocketServer(PORT_CAT_CONTENT_SERVICE);
  safeListen(service_socket);

  LOG_INFO("Service CAT CONTENT FILES started. Waiting for connections...");
...
void *host_handler(void *socket_desc)
{
  ...
  if (message_rcv.serviceType == CAT_DIR)
  {
    ListCatPayload cat_dirs = {0};
    strcpy(cat_dirs.directory_path, message_rcv.payload.listCatPayload.directory_path);
    // TODO check the error status of the list dir to see if the op is success or not ? 
    getFileContent(cat_dirs.directory_path, cat_dirs.output, sizeof(cat_dirs.output));
    message_send.serviceType = CAT_DIR;
    message_send.payload.listCatPayload = cat_dirs;
